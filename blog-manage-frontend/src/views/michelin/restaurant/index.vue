  ...
  <div>
    <vxe-grid
      ...
      <template v-slot:name_default="{ row, column,seq }">
        <vxe-button status="info" @click="showDetailEvent(row, column)">查看</vxe-button>
        <vxe-button status="primary" @click="editDetailEvent(row, column)" >修改</vxe-button>
        <vxe-button status="danger" @click="deleteById(row)" >删除</vxe-button>
      </template>
    </vxe-grid>
    <div>
      <el-dialog
